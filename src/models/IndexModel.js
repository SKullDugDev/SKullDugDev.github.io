...
exports.populateInstafeed = async (req, res) => {
  // Make the API Call
  app.get("/instafeedResults", (req, res) => {
    const instafeedResults = app.get("instafeedResults");
    return instafeedResults;
  });

  // return the info for the images
  const imageInfo = await Promise.all(
    Object.entries(instafeedResults.data).map(
      async ([key, apiResponseData]) => {
